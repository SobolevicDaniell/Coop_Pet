...
namespace Game.UI
{
    public sealed class MenuUIController : MonoBehaviour
    {
        ...
        {
            if (_exitToMenuButton != null) _exitToMenuButton.onClick.RemoveListener(OnExitToMenuClicked);
            if (_quitGameButton != null) _quitGameButton.onClick.RemoveListener(OnQuitGameClicked);
        }
        ...
        {
            if (_exitToMenuButton != null) _exitToMenuButton.interactable = value;
            if (_quitGameButton != null) _quitGameButton.interactable = value;
        }

        private List<NetworkRunner> CollectRunners()
        {
            var list = new List<NetworkRunner>();
            if (_runner != null) list.Add(_runner);
            var found = FindObjectsOfType<NetworkRunner>(true);
            ...
            return list.Where(r => r != null).ToList();
        }
    }
}
